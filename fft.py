...
freqs = nf.fftfreq(times.size, times[1]-times[0])        # 生成频率集，与时间哉的采样点相等; 时间哉的采样周期
ffts = nf.fft(sigs6)
print(ffts)
pows = np.abs(ffts)        # 求复数的长度，为能量值
sigs7 = nf.ifft(ffts)      # 傅里叶反变换
...
mp.plot(times, sigs7, label='{:.4f}'.format(1/(2*np.pi)), alpha=0.5, linewidth=3)
mp.legend()
mp.subplot(122)
mp.title('Freq Domain', fontsize=16)
mp.xlabel('Freq', fontsize=12)
