...
N = 5
medios = np.convolve(
    closing_prices, np.ones(N) / N, 'valid')
...
lowers = medios - stds
uppers = medios + stds
mp.figure('Simple Bollinger Bands',
          facecolor='lightgray')
mp.title('Simple Bollinger Bands', fontsize=20)
...
mp.plot(dates[4:], medios, c='dodgerblue',
        linewidth=3, label='Medio')
mp.plot(dates[4:], lowers, c='limegreen',
        linewidth=3, label='Lower')
mp.plot(dates[4:], uppers, c='orangered',
        linewidth=3, label='Upper')
mp.legend()
